...
| 美观 | 封面、目录、色块、分节，完整配色 | — |
| 极简 | 只保留 Markdown 语法能表达的格式：标题、列表、表格、加粗斜体 | 颜色、底纹、边框、自定义字体等特殊排版 |

用户没指定，就按"普通"来。

极简也是 `.docx`，只是样式收敛到 Markdown 级别：
仍用 `add_h1`/`add_h2`/`add_body`/`add_list_item`/`add_data_table`，
但 `set_run_font` 不传颜色、不加底纹和边框。

...
h1_bar 三、分段点评（批注式）  → 以旁批为主：side_table（左原文、右批注）+ 段内批【】
...
第三节“分段点评”一律用批注式，完整展示原文（一字不落），并以旁批为主：

| 形式 | 用在 | docx 实现 |
|---|---|---|
| 旁批（主力） | 逐段对照，每个主体段一条 | `side_table`（两列：左原文、右批注） |
| 段内批 | 局部字句、错别字、逻辑跳步 | 左列原文里直接插 `【...】` |
| 段末批 | 整段的总结性判断 | 原文段之后另起一段 body/block |

做法：把各段原文按顺序放进 `side_table` 左列（局部问题用 `【】` 标出），右列写批注；
需要整段总评时再补一段段末批。这样原文完整、批注贴行，读者一段一段对着看。
